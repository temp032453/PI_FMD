...
def reorder(real_data, syn_data, info):
    ...
    for i in range(len(new_real_data.columns)):
        if i < len(num_col_idx):
            ...
            new_real_data.loc[new_real_data[i] == '?', i] = np.nan
            new_syn_data.loc[new_syn_data[i] == '?', i] = np.nan
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target', type=str, default='priv') # priv, pub, all
    ...
    targets = [ 'priv', 'pub', 'all']
    for target in targets:
        from sdmetrics.reports.single_table import QualityReport, DiagnosticReport
        if target == 'priv':
            # real_path ='./csv_arxiv/origin_priv_True.csv'
            real_path =f'./csv_arxiv/{dataname}_origin_priv_True.csv'
        elif target == 'pub':
            real_path =f'./csv_arxiv/{dataname}_origin_priv_False.csv'
        elif target == 'all':
            real_path =f'./csv_arxiv/{dataname}_origin_all.csv'
    
        real_data = pd.read_csv(real_path)
        syn_data = pd.read_csv(syn_path)
        save_dir = f'eval/density/{dataname}/{model}_{target}'
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        with open(f'./csv_arxiv/{dataname}_info.json', 'r') as f:
            info = json.load(f)
        ...
        shapes_scores.append(Shape.round(3))
        trends_scores.append(Trend.round(3))
        ...
        del qual_report, Shape, Trend, Quality, shapes, trends, real_data, syn_data, new_real_data, new_syn_data, metadata

    print(f'targets : {targets}')
    print(f'col-wise : {shapes_scores}')
    print(f'pair-wise : {trends_scores}')
