...
class MLPDiffusion(nn.Module):
    def __init__(self, d_in, dim_t = 512):
        ...
        self.fixed_weight_init()

    def fixed_weight_init(self):
        for m in self.modules():
            if isinstance(m, nn.Linear):  # Apply to Linear layers
                #torch.manual_seed(1234)  # Fix the seed for consistent initialization
                torch.nn.init.xavier_uniform_(m.weight)  
                m.bias.data.fill_(0.1)   # Set all biases to 0.1
...
class Model(nn.Module):
    def __init__(self, denoise_fn, hid_dim, P_mean=0, P_std=1.2, sigma_data=0.5, gamma=5, opts=None, pfgmpp = False):
        super().__init__()
